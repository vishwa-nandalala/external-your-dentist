...
/**
 * Try to load a clinic profile.
 * 1. First attempt: claimed clinic via getClinicById
 * 2. Fallback:     unclaimed practice via getUnclaimedPracticeById
 * Returns null if neither exists.
 */
async function loadClinic(id: string) {
  // 1. Try claimed clinic
  try {
    const clinic = await practiceApi.getClinicById(id);
    if (clinic) return clinic;
  } catch {
    // ignore — fall through to unclaimed
  }

  // 2. Fallback to unclaimed practice (mapped to ClinicProfile shape with status: "INACTIVE")
  try {
...
export default async function ClinicProfilePage({ params }: Props) {
  ...
  // Try claimed clinic first, then fall back to unclaimed practice.
  const clinic = await loadClinic(id);
